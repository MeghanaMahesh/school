...
def parent_feedback(request):
    staff_id = Parents.objects.get(admin=request.user.id)
    feedback_data = FeedBackParent.objects.filter(parent_id=staff_id)
...
def parent_feedback_save(request):
    if request.method != "POST":
        ...
        student_obj = Parents.objects.get(admin=request.user.id)
        try:
            feedback = FeedBackParent(parent_id=student_obj, feedback=feedback_msg, feedback_reply="")
            feedback.save()
